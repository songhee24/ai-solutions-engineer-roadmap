...
test("финиш считается по рабочим дням, а не по календарным", () => {
  ...
  assert.equal(finishDate(p, 10, "2026-09-04"), "2026-09-07");
});

test("уже отработанные сегодня часы уменьшают остаток первого дня", () => {
  const p = planner();
  assert.equal(finishDate(p, 5, "2026-09-01", 0), "2026-09-01");
  assert.equal(finishDate(p, 5, "2026-09-01", 3), "2026-09-02", "сегодня осталось 2 часа из 5");
});
