...
for (const s of ["math", "english", "seq"]) {
  ...
  out.push(`| ${STREAMS[s].title} | ${Math.round(total)} | **${formatHours(budget.perDay[s])}** | ${(budget.perDay[s] * 7).toFixed(1)} ч |`);
}
out.push(`| **Итого** | **${Math.round(budget.totalHours)}** | **${HOURS} ч** | ${HOURS * 7} ч |`);
out.push("");
out.push(`Весь основной путь при этом темпе — **${budget.days} дней ≈ ${(budget.days / 30.44).toFixed(1)} месяцев**`);
...
console.log(`  весь путь: ${budget.days} дней, ${Math.round(budget.totalHours)} ч`);
